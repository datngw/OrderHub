...
public class AddBasketItemCommandValidatorTests
{
    private readonly AddBasketItemCommandValidator _validator = new();

    [Fact]
    public async Task Validate_WithValidCommand_ShouldHaveNoErrors()
    {
        var command = new AddBasketItemCommand(Guid.NewGuid(), 5);
    ...
    {
        var command = new AddBasketItemCommand(Guid.Empty, 5);
        var result = await _validator.TestValidateAsync(command);
    ...
    {
        var command = new AddBasketItemCommand(Guid.NewGuid(), quantity);
        var result = await _validator.TestValidateAsync(command);
